...
public class Jury extends HttpServlet {
  ...
  void evaluate(HttpServletRequest request, HttpServletResponse response, String university) throws IOException, ServletException {
    ...
    try {
      ...
      for (int juryIndex = 0; juryIndex <= nbJudges; ++juryIndex) {
        query = "CREATE TABLE IF NOT EXISTS candidates_" + university + "_jury_" + juryIndex + " AS SELECT `Ma so`, `Ho dem`, `Ten`, "
        ...
        resultSet = statement.executeQuery("SELECT ROUND(AVG(hoancanh),2) AS hoancanh, ROUND(AVG(hoctap),2) AS hoctap, ROUND(AVG(uocmo),2) AS uocmo, ROUND(AVG(diemcong),2) AS diemcong, "
        ...
        resultSet = statement.executeQuery("SELECT ROUND(STDDEV(hoancanh),2) AS hoancanh, ROUND(STDDEV(hoctap),2) AS hoctap, ROUND(STDDEV(uocmo),2) AS uocmo, ROUND(STDDEV(diemcong),2) AS diemcong, "
