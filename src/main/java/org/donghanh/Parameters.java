...
public class Parameters extends HttpServlet {
	...
	*/
	...
	void viewParams(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		out.println(doctype);
		out.println("<html>");
		out.println("<head><title>" + title + "</title><meta charset=\"UTF-8\" /><script type=\"text/javascript\" src=\"normalize.js\" ></script>"
		...
		out.println("<tr>");
		out.println("<th>Tham số</th><th>Giá trị</th>");
		out.println("</tr>");
		
		for (int index = 0; index < PARAMS_LIST.length; ++index){
			if (SECURITY[index] == 0){
				out.println("<tr>");
				out.println("<td>" + PARAMS_LIST[index] + "</td><td><input type='text' size='80' id='input_" + PARAMS_LIST[index] + "' value='" + PARAMS.get(PARAMS_LIST[index]) + "'></input><"
							+ "button type='button' onclick='set_params(\"" + PARAMS_LIST[index] + "\")'>Cập nhật</button><p><a href='' id='confirm_" + PARAMS_LIST[index] + "'></a></p></td>");
				out.println("</tr>");
			}
		...
		out.println("<table width=\"100%\" border=\"1\" align=\"center\">");
		out.println("<tr>");
		out.println("<th>Mã trường</th><th>Tên trường</th><th>Tên quỹ học bổng</th><th>Đối tượng</th><th>Nơi chấm</th><th>Logo</th><th>Hệ số VN</th><th>Số gk/hồ sơ</th><th>Số hồ sơ max</th><th></th>");
		out.println("</tr>");
		
		for (String key: FOUNDATION_NAMES.keySet()){
			out.println("<tr>");
			out.println("<td>" + key + "</td>"
					+ "<td><input type='text' size='30' id='UniversityName_" + key + "' value='" + UNIVERSITY_NAMES.get(key) + "'></input></td>"
					+ "<td><input type='text' size='20' id='FoundationName_" + key + "' value='" + FOUNDATION_NAMES.get(key) + "'></input></td>"
					+ "<td><input type='text' size='10' id='StudentClass_" + key + "' value='" + STUDENT_CLASS.get(key) + "'></input></td>"
					+ "<td><input type='text' size='5' id='EvaluatedBy_" + key + "' value='" + EVALUATED_BY.get(key) + "'></input></td>"
					+ "<td><input type='text' size='20' id='Logo_" + key + "' value='" + LOGO.get(key) + "'></input></td>"
					+ "<td><input type='text' size='5' id='VnCoefs_" + key + "' value='" + VN_COEFS.get(key) + "'></input></td>"
					+ "<td><input type='text' size='5' id='NbJugesByCopy_" + key + "' value='" + NB_JUGES_BY_COPY.get(key) + "'></input></td>"
					+ "<td><input type='text' size='5' id='MaxDocs_" + key + "' value='" + MAX_DOCS.get(key) + "'></input></td>"
					+ "<td><button type='button' size='10' onclick='set_university_params(\"" + key + "\")'>Cập nhật</button><p><a href='' id='confirm_university_" + key + "'></a></p></td>");
			out.println("</tr>");
		}
		out.println("</table>");
